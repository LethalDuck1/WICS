...
export function extractShowTitles(text: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (!candidates.has(normalized)) {
      candidates.set(normalized, titleCase(line.trim()));
    }
  }
...
function findPossibleMatch(
  ...
  for (const other of others) {
    ...
    if (normalizedTitle === normalizedOther) {
      return other;
    }
    ...
    const tokenOverlap = normalizedTitle
      .split(" ")
      .filter((token) => token && normalizedOther.includes(token)).length;

    const strongEnough = score >= 0.72 || (score >= 0.6 && tokenOverlap >= 2);
    if (!strongEnough) continue;

    if (!best || score > best.score) {
      best = { value: other, score };
    }
  }
...
export function compareInventories(
  ...
  const prevNormalized = new Map(
    previousTitles.map((title) => [normalizeShowTitle(title), title])
  );

  const currNormalized = new Map(
    currentTitles.map((title) => [normalizeShowTitle(title), title])
  );
  ...
  for (const current of currentTitles) {
    ...
    if (!existsInPrevious) {
      results.push({ title: current, status: "added" });
    }
  }
